...
export const useStudioStore = create<StudioState>((set, get) => ({
  ...
  generateWorkflowFromPrompt: async (prompt) => {
    ...
    try {
      ...
      get().addSimulationLog({
        type: 'error',
        message: `Falha na geração real de fluxo: ${err.message}. Revertendo para modelo simulado offline local...`
      });

      // Local mock fallback logic so it never fails visually for the user
      const isClinic = prompt.toLowerCase().includes('clínica') || prompt.toLowerCase().includes('odonto') || prompt.toLowerCase().includes('médico') || prompt.toLowerCase().includes('consulta');
      
      let generatedNodes: StudioNode[] = [];
      let generatedEdges: StudioEdge[] = [];
      ...
      if (isClinic) {
        generatedNodes = [
          {
            ...
            type: 'start',
            position: { x: 50, y: 300 },
            data: { label: 'Inbound Call (Clínica)', category: 'Trigger', config: nodeRegistry.start.defaultConfig, metrics: { invocations: 10, errorRate: 0, latencyMs: 12 } }
          },
          {
            id: 'voice-1',
            type: 'voice',
            position: { x: 380, y: 150 },
            data: { label: 'Voz da Recepcionista', category: 'Config', config: { ...nodeRegistry.voice.defaultConfig, voiceId: 'Rachel_pt_BR' }, metrics: { invocations: 10, errorRate: 0, latencyMs: 8 } }
          },
          {
            id: 'llm-1',
            type: 'llm',
            position: { x: 380, y: 450 },
            data: { label: 'Gemini Clínico', category: 'LLM', config: { ...nodeRegistry.llm.defaultConfig, temperature: 0.1 }, metrics: { invocations: 10, errorRate: 0.02, latencyMs: 145 } }
          },
          {
            id: 'prompt-1',
            type: 'prompt',
            ...
            data: { 
              ...
              category: 'Prompt', 
              config: { 
                promptText: 'Você é a assistente virtual da Birth Dental. Agende horários de consultas, valide o CPF do paciente, consulte horários livres e responda dúvidas gerais.', 
                streaming: 'Enabled', 
                thinking: 'DeepThinking' 
              }, 
              metrics: { invocations: 10, errorRate: 0.05, latencyMs: 420 } 
            }
          },
          {
            id: 'question-1',
            ...
            data: { 
              ...
              category: 'Conversation', 
              config: { 
                questionText: 'Por favor, fale ou digite o número do seu CPF.', 
                validationRegex: '^[0-9]{11}$', 
                variableToSave: 'customer_cpf' 
              }, 
              metrics: { invocations: 8, errorRate: 0, latencyMs: 20 } 
            }
          },
          {
            id: 'tool-1',
            ...
            data: { 
              label: 'API Validar CPF', 
              category: 'Integration', 
              config: { 
                method: 'POST', 
                endpoint: 'https://api.birthdental.com/v1/validate-patient', 
                bodyPayload: '{"cpf": "{{customer_cpf}}"}' 
              }, 
              metrics: { invocations: 5, errorRate: 0.1, latencyMs: 350 } 
            }
          },
          {
            id: 'knowledge-1',
            ...
            data: { 
              label: 'FAQ Procedimentos', 
              category: 'Knowledge', 
              config: { database: 'Notion FAQs' }, 
              metrics: { invocations: 3, errorRate: 0, latencyMs: 290 } 
            }
          },
          {
            id: 'end-1',
            ...
            data: { label: 'Finalizar Atendimento', category: 'Trigger', config: nodeRegistry.end.defaultConfig, metrics: { invocations: 3, errorRate: 0, latencyMs: 10 } }
          }
        ];
        ...
        generatedEdges = [
          ...
          { id: 'e-p1-q1', source: 'prompt-1', target: 'question-1', type: 'studioEdge' },
          { id: 'e-q1-t1', source: 'question-1', target: 'tool-1', sourceHandle: 'out-0', type: 'studioEdge', data: { condition: 'CPF Válido' } },
          { id: 'e-q1-k1', source: 'question-1', target: 'knowledge-1', sourceHandle: 'out-1', type: 'studioEdge', data: { condition: 'CPF Inválido / Dúvida', isFallback: true } },
        ...
        generatedNodes = [
          {
            ...
            type: 'start',
            position: { x: 50, y: 300 },
            data: { label: 'Inbound Call General', category: 'Trigger', config: nodeRegistry.start.defaultConfig, metrics: { invocations: 1, errorRate: 0, latencyMs: 12 } }
          },
          {
            id: 'prompt-1',
            type: 'prompt',
            position: { x: 450, y: 300 },
            data: { label: 'Prompt Geral Assistant', category: 'Prompt', config: { promptText: `Fluxo criado a partir do prompt do usuário: "${prompt}"` }, metrics: { invocations: 1, errorRate: 0, latencyMs: 250 } }
          },
          {
            id: 'end-1',
            type: 'end',
            position: { x: 850, y: 300 },
            data: { label: 'Finalizar', category: 'Trigger', config: nodeRegistry.end.defaultConfig, metrics: { invocations: 1, errorRate: 0, latencyMs: 10 } }
          }
        ];
        
        generatedEdges = [
          { id: 'e1-p1', source: 'start-1', target: 'prompt-1', type: 'studioEdge' },
          { id: 'ep1-end', source: 'prompt-1', target: 'end-1', type: 'studioEdge' }
        ];
      }
      ...
      get().addSimulationLog({
        type: 'success',
        message: `Geração simulada offline finalizada com sucesso (${generatedNodes.length} nós).`
      });
    }
  },
